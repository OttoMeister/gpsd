...
void get_gps_text(){
  ...
  if (gps_open(GPSD_SHARED_MEMORY, NULL, &gps_data) != 0) {
    (void)sprintf(gps_text1, "gpsd: no gpsd running or network error: %d, %s\n",
                  errno, gps_errstr(errno));
  } else {
    // Blocking read for gps data from the daemon socket
    gps_read(&gps_data);
    // Start collecting text
    sprintf(gps_text1, "GPSstatus: ");
    // fix cps conditions
    switch (gps_data.fix.mode) {
      case MODE_3D:
        sprintf(gps_text1, "%s3d-fix", gps_text1);
        break;
      case MODE_2D:
        sprintf(gps_text1, "%s2d-fix", gps_text1);
        break;
      case MODE_NO_FIX:
        sprintf(gps_text1, "%sno-fix", gps_text1);
        break;
      default:
        sprintf(gps_text1, "%sempty-fix", gps_text1);
        break;
    }
    // local gps time
    aclock = gps_data.fix.time;
    sprintf(gps_text1, "%s, GPStime: %s", gps_text1, ctime(&aclock));


    // gps position
    sprintf(gps_text2,"lat: %f", gps_data.fix.latitude);
    sprintf(gps_text2,"%s%c, ", gps_text2, (gps_data.fix.latitude < 0) ? 'S' : 'N');
    sprintf(gps_text2,"%slon: %f", gps_text2, gps_data.fix.longitude);
    sprintf(gps_text2,"%s%c, ", gps_text2, (gps_data.fix.longitude < 0) ? 'W' : 'E');
    // gps speed
    sprintf(gps_text2, "%sspeed: %.2f km/h", gps_text2, 3.6 * gps_data.fix.speed);  
...
draw_text(cnt->current_image->image, cnt->imgs.width - 10,
                          cnt->imgs.height - 30 * text_size_factor,
                          cnt->imgs.width, gps_text1, cnt->conf.text_double);

draw_text(cnt->current_image->image, cnt->imgs.width - 10,
                          cnt->imgs.height - 20 * text_size_factor,
                          cnt->imgs.width, gps_text2, cnt->conf.text_double);
